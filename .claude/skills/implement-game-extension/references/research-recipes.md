...
- **xbox** — the package **Identity Name** (the part before `_<publisherhash>`), NOT the `9N…` store
  ...
  **Sanity-check the shape before using it.** A real identity looks like a product name
  (`rokapublish.Solarpunk`, `UnknownWorldsEntertainmen.Subnautica2`). An autogenerated numeric one
  like `Microsoft.198377053870B` is the pattern used for **pre-release placeholders** and may change
  at launch — record it in a comment and leave `xbox:` out until it can be confirmed against a real
  install (`Get-AppxPackage *<Name>*`, or the `appsFolder` key). See
  `games/halocampaignevolved/game.yaml` for that pattern written up.

  **`xbox:` is rarely sufficient on its own.** Game Pass builds put binaries under `Binaries/WinGDK`
  rather than `Binaries/Win64`, and `requiredFiles` takes literal paths with no glob expansion — so a
  Win64-only `requiredFiles` will fail discovery on Xbox even with the correct store id. Add the
  store and the WinGDK path together, or neither.
